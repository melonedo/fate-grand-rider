...
void multipleInheritance::load(const std::string& name, Weapon* weaponLoading,
...
void multipleInheritance::loadfire(Weapon* weaponLoading,
                                   const rapidjson::Value& weapon_data) {
  weaponLoading->_fireweapon = Sprite::create();
...
BlinkBow* BlinkBow::createweapon(const std::string& name) {
  ...
  blinkbow->multipleInheritance::load(name, blinkbow, data, blinkbow_data);
 ...
 /* blinkbow->_arrow = Sprite::create();
  ...
  blinkbow->_fireweapon->setAnchorPoint(
      Vec2(anchor_data2[0].GetFloat(), anchor_data2[1].GetFloat()));//两次加载的相同图片，bug警告
...
void Bow::pointTo(Vec2 offset) {
  float angle_offset = isFlippedX() ? 180 - _weaponAngleOffset : _weaponAngleOffset;
...
Magic* Magic::createweapon(const std::string& name) {
  ...
  magic->loadfire(magic, magic_data);
